CLASS zcl_dt266_carr_extension_new DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZCL_DT266_CARR_EXTENSION_NEW IMPLEMENTATION.
...
*    out->write( |[DT266] Run finished for ZCL_DT266_LOOP1_{ group_id }. | ).
  ...
  METHOD get_supplements_ABAP.
    SELECT supplement_id,id, supplement_category, price FROM zdt266_sup_i_000
...
* Exercise 2.1: Memory Analysis -End
    ...
    lt_supplement_sort2 = lt_supplement.
...
*        LOOP AT lt_book_suppl_sort INTO ls_book_suppl
*        WHERE travel_id = ls_booking-travel_id AND booking_id = ls_booking-booking_id.
*            LOOP AT lt_supplement_key INTO ls_supplement
