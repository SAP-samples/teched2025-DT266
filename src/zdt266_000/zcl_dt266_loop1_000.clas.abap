CLASS zcl_dt266_loop1_000 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_book_suppl,
             travel_id     TYPE /dmo/travel_id,
             booking_id    TYPE /dmo/booking_id,
*Exercise 4.5: Compare with FAE -START
**********************************************************************
*             booking_supplement_id TYPE /dmo/booking_supplement_id,
*Exercise 4.5: Compare with FAE -END
**********************************************************************
  ...
  PRIVATE SECTION.
    ...
    METHODS get_supplements_ABAP IMPORTING lt_id               TYPE tt_id
                                           supplement_category TYPE  /dmo/supplement_category
...
CLASS ZCL_DT266_LOOP1_000 IMPLEMENTATION.
...
*    out->write( |[DT266] Run finished for ZCL_DT266_LOOP1_{ group_id }. | ).
  ...
  METHOD get_supplements_ABAP.
    SELECT supplement_id,id, supplement_category, price FROM (table_supplement)
...
*    APPENDING TABLE @DATA(lt_book_suppl2).
**********************************************************************
* Exercise 4.5: Compare with FAE -END

* Exercise 4.5: Table Comparison Tool -START
**********************************************************************

    SORT lt_book_suppl BY travel_id booking_id supplement_id.

*    SELECT travel_id, booking_id, booking_supplement_id, supplement_id, price FROM /dmo/book_suppl
*        APPENDING TABLE @DATA(lt_book_suppl2).
*    SORT lt_book_suppl2 BY travel_id booking_id supplement_id.

* Sorted tables
    lt_book_suppl_sort = lt_book_suppl.
**********************************************************************
* Exercise 4.5: Table Comparison Tool --END
...
* Exercise 4.2: Table Buffering -START
**********************************************************************
* Get the prices and categories of the booked supplements
    LOOP AT lt_book_suppl INTO ls_book_suppl.
      SELECT supplement_id,id, supplement_category, price FROM (table_supplement)
      WHERE supplement_id = @ls_book_suppl-supplement_id
      AND id = 1
      APPENDING TABLE @lt_supplement2.
    ENDLOOP.

    SORT lt_supplement2 BY supplement_id id.
    DELETE ADJACENT DUPLICATES FROM lt_supplement2 COMPARING supplement_id id.
    ...
    WHILE id < 499.
      ...
      CALL METHOD get_supplements_ABAP EXPORTING lt_id = lt_id supplement_category = 'LU' table_supplement = table_supplement
      CHANGING lt_supplement = lt_supplement2.
*      SELECT supplement_id,id, supplement_category, price FROM (table_supplement)
*      FOR ALL ENTRIES IN @lt_id
*      WHERE supplement_category = 'LU' AND id = @lt_id-id
*      APPENDING TABLE @lt_supplement2.
      id = id + 1.
    ...
    WHILE id < 499.
      ls_id-id = id.
      APPEND ls_id TO LT_id.
      CALL METHOD get_supplements_ABAP EXPORTING lt_id = lt_id supplement_category = 'BV' table_supplement = table_supplement
      CHANGING lt_supplement = lt_supplement2.
*      SELECT supplement_id,id, supplement_category, price FROM (table_supplement)
*      FOR ALL ENTRIES IN @lt_id
*      WHERE supplement_category = 'BV' AND id = @lt_id-id
*      APPENDING TABLE @lt_supplement2.
      id = id + 1.
      APPEND LINES OF lt_supplement2 TO lt_supplement.
      CLEAR lt_id.
    ...
    WHILE id < 499.
      ls_id-id = id.
      APPEND ls_id TO LT_id.
      CALL METHOD get_supplements_ABAP EXPORTING lt_id = lt_id supplement_category = 'ML' table_supplement = table_supplement
      CHANGING lt_supplement = lt_supplement2.
*      SELECT supplement_id,id, supplement_category, price FROM (table_supplement)
*      FOR ALL ENTRIES IN @lt_id
*      WHERE supplement_category = 'ML' AND id = @lt_id-id
*      APPENDING TABLE @lt_supplement2.
      id = id + 1.
      APPEND LINES OF lt_supplement2 TO lt_supplement.
      CLEAR lt_id.
...
**********************************************************************
* Exercise 4.2: Table Buffering -End
    ...
    SORT lt_supplement BY supplement_id id supplement_category price.
    SORT lt_book_suppl BY travel_id booking_id supplement_id.
...
* Exercise 4.4: Nested Loops -START
**********************************************************************
...
*      LOOP AT lt_booking INTO ls_booking WHERE carrier_id = ls_carrier-carrier_id.
*        READ TABLE lt_book_suppl TRANSPORTING NO FIELDS
*        WITH KEY travel_id = ls_booking-travel_id  booking_id = ls_booking-booking_id BINARY SEARCH.
*        l_tabix_1 = sy-tabix.
*        LOOP AT lt_book_suppl INTO ls_book_suppl FROM l_tabix_1.
...
*            WITH KEY supplement_id = ls_book_suppl-supplement_id  id = 1 BINARY SEARCH.
*            l_tabix_2 = sy-tabix.
*            LOOP AT lt_supplement INTO ls_supplement FROM l_tabix_2.
...
* Exercise 4.4: Nested Loops -End
  ...
  METHOD get_prices_CDS.
    ...
    DATA:
      ...
      suppl_price_sum_lugg TYPE /dmo/supplement_price,
      flight_price_sum     TYPE /dmo/supplement_price,
*      suppl_price_sum      TYPE p DECIMALS 4,
*      suppl_price_sum_meal TYPE p DECIMALS 4,
*      suppl_price_sum_bev  TYPE p DECIMALS 4,
*      suppl_price_sum_lugg TYPE p DECIMALS 4,
*      flight_price_sum     TYPE p DECIMALS 4,
      group_id             TYPE string VALUE '000',
      l_tabix_1            TYPE sy-tabix,
      l_tabix_2            TYPE sy-tabix,
      lt_booking_sort      TYPE SORTED TABLE OF ty_booking WITH NON-UNIQUE KEY primary_key  COMPONENTS carrier_id,
      lt_book_suppl_sort   TYPE SORTED TABLE OF ty_book_suppl WITH NON-UNIQUE KEY primary_key  COMPONENTS travel_id booking_id,
      lt_book_suppl_sort2  TYPE SORTED TABLE OF ty_book_suppl WITH NON-UNIQUE KEY primary_key  COMPONENTS travel_id booking_id,
      lt_supplement_sort   TYPE SORTED TABLE OF ty_supplement WITH NON-UNIQUE KEY primary_key COMPONENTS supplement_id
      WITH NON-UNIQUE SORTED KEY key_id_cat COMPONENTS id supplement_category,
      lt_carrier_price_sum TYPE STANDARD TABLE OF ty_carrier_price_sum.
    ...
    SELECT
     ...
     FROM
      z_i_price
      FOR ALL ENTRIES IN @lt_carrier WHERE carrierid = @lt_carrier-carrier_id AND id = 1
  ...
  METHOD if_sadl_exit_calc_element_read~calculate.
    ...
    DATA lt_original_data TYPE STANDARD TABLE OF zc_dt266_carr_000 WITH DEFAULT KEY.
